...
public class CommandProcessor {
    ...
    private String select(String command) throws DbExceptions.DataManipulationException {
        ...
        if (!mainTableMatcher.find())
            throw new DbExceptions.DataManipulationException("Incorrect SELECT syntax");
        Table mainTable = usedDatabase.getTable(mainTableMatcher.group(1));
        if(mainTable == null)
            throw new DbExceptions.DataManipulationException(
                    "Can't select from non-existent table: " + mainTableMatcher.group(1));
        tableAS.put(mainTableMatcher.group(1), mainTable);
        if (mainTableMatcher.group(2) != null)
            tableAS.put(mainTableMatcher.group(2), mainTable);
        selectedTables.add(mainTable);

        indexAttributesInTable(mainTable, attributeByNameInTable, indexOfAttribute);
        ...
        if (joins != null) {
            ...
            while (joinedTableMatcher.find()) {
                ...
                if (!joinedTableNameMatcher.find())
                    throw new DbExceptions.DataManipulationException("Incorrect JOIN syntax in SELECT");
                Table joinedTable = usedDatabase.getTable(joinedTableNameMatcher.group(1));
                if (joinedTable == null)
                    throw new DbExceptions.DataManipulationException(
                            "Can't join non-existent table: " + joinedTableNameMatcher.group(1));
                tableAS.put(joinedTableNameMatcher.group(1), joinedTable);
                if (joinedTableNameMatcher.group(2) != null)
                    tableAS.put(joinedTableNameMatcher.group(2), joinedTable);
                selectedTables.add(joinedTable);

                indexAttributesInTable(joinedTable, attributeByNameInTable, indexOfAttribute);
                ...
                Table table1 = usedDatabase.getTable(tableName1);
                if (table1 == null)
                    throw new DbExceptions.DataManipulationException("Table not exists: " + tableName1);
                Table table2 = usedDatabase.getTable(tableName2);
                if (table2 == null)
    ...
    private String getTableOfAttribute(List<Table> selectedTables, String attributeName) {
    ...
    private void indexAttributesInTable(
            Table table,
    ...
    private List<List<String>> findFromTableByFilters(
            Table table,
